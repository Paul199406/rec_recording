...
#define sample_count_per_channel  2025/2/4
...
static AVCodecContext* open_coder(){
    ...
    codec_ctx->sample_fmt = AV_SAMPLE_FMT_S16;          //输入音频的采样大小在libfdk_aac要求设置为  AV_SAMPLE_FMT_S16
    ...
    codec_ctx->profile = FF_PROFILE_AAC_HE_V2; //阅读 ffmpeg 代码 设置profile后并不需要设置bit_rate,需要设置bit_rate为0
...
AVFrame* create_frame(){
    ...
    //set parameters
    frame->nb_samples     = sample_count_per_channel;                //单通道一个音频帧的采样数
...
void alloc_data_4_resample(uint8_t ***src_data,
    ...
    av_samples_alloc_array_and_samples(src_data,         //输入缓冲区地址
                                       src_linesize,     //缓冲区的大小
                                       2,                 //通道个数
                                       sample_count_per_channel,               //单通道采样个数
...
void read_data_and_encode(AVFormatContext *fmt_ctx, //
    ...
    while((ret = av_read_frame(fmt_ctx, &pkt)) == 0 && rec_status) {
        ...
        swr_convert(swr_ctx,                    //重采样的上下文
                    dst_data,                   //输出结果缓冲区
                    sample_count_per_channel,                        //每个通道的采样数
                    (const uint8_t **)src_data, //输入缓冲区
                    sample_count_per_channel);                       //输入单个通道的采样数
...
void rec_audio() {
    ...
    char *out = "/Users/jiweili/Desktop/macapp/macapp/audio.aac";
